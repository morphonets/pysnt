...
def setup_module_classes(
    ...
    def _java_setup():
        ...
        try:
            ...
            for class_name in curated_classes:
                ...
                for pkg in discovery_packages:
                    try:
                        ...
                        logger.debug(f"Loaded curated class {class_name} from {pkg}")
                        break
                    except Exception:
                        continue
                ...
                if java_class is not None:
                    ...
                    globals_dict[class_name] = java_class
                else:
            ...
            raise ImportError(f"Could not load curated classes for {package_name}: {e}") from e
                ...
                for class_name in discovered_classes:
                    if class_name not in _curated_classes:
                        try:
                            ...
                            logger.debug(f"Discovered extended class: {class_name} from {pkg}")
                            
                        except Exception as e:
                            logger.warning(f"Failed to load extended class {class_name} from {pkg}: {e}")
            
            _discovery_completed = True
            logger.info(f"Discovered {len(_extended_classes)} extended classes for {package_name}")
    ...
    def create_getattr(module_name: str, submodules: List[str] = None) -> Callable[[str], Any]:
        """
