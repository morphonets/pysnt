...
EXTENDED_CLASSES = [
    ...
    "StrahlerAnalyzer",
    "ShollAnalyzer",
    "DirectionalityAnalyzer",
    "PersistenceAnalyzer",
    "RamerDouglasPeuckerSimplifier",
    "GraphColorMapper",
    "TreeStatistics2",
    "SNTChart",
...
def _java_setup():
    """
    Lazy initialization function for Java-dependent analysis classes.
    ...
    try:
        ...
        logger.error(f"Failed to load curated analysis classes: {e}")
        raise ImportError(f"Could not load curated analysis classes: {e}") from e
...
def _discover_extended_classes():
    ...
    try:
        ...
        logger.info(f"Discovered {len(_extended_classes)} extended analysis classes")
...
def get_class(class_name: str) -> Any:
    ...
    >>> TreeStats = get_class("TreeStatistics")
    >>> 
    >>> # Get an extended class (discovered on-demand)
    >>> NodeStats = get_class("NodeStatistics")
    """
    if not scyjava.jvm_started():
        raise RuntimeError(
...
def list_classes():
    ...
    print("Available SNT Analysis Classes:")
    print("=" * 40)
