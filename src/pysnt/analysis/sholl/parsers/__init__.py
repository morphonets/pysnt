...
`SNT's Sholl parser classes <https://javadoc.scijava.org/SNT/index.html?sc/fiji/snt/analysis/sholl/parsers/package-summary.html>`__.
"""
...
def _java_setup():
    ...
    """
    global _curated_classes, ImageParser, TreeParser, ImageParser2D, ImageParser3D
    
    try:
        package_name = "sc.fiji.snt.analysis.sholl.parsers"
        ...
        if "ImageParser2D" in _curated_classes:
            globals()["ImageParser2D"] = _curated_classes["ImageParser2D"]
        if "ImageParser3D" in _curated_classes:
            globals()["ImageParser3D"] = _curated_classes["ImageParser3D"]
        if "TreeParser" in _curated_classes:
            globals()["TreeParser"] = _curated_classes["TreeParser"]
        
        logger.info(f"Successfully loaded {len(_curated_classes)} curated Sholl parser classes")
...
def _discover_extended_classes():
    ...
    try:
        ...
        logger.info(f"Discovered {len(_extended_classes)} extended Sholl parser classes")
...
def list_classes():
    ...
    print("Available SNT Sholl Parser Classes:")
    print("=" * 40)
...
def __getattr__(name: str) -> Any:
    """
    Provide dynamic access to Sholl parser classes.
...
class ImageParser2D:
    """
    Curated SNT class from analysis/sholl/parsers package with complete method signatures.
