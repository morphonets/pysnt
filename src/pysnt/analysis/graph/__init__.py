...
`SNT's graph analysis classes <https://javadoc.scijava.org/SNT/index.html?sc/fiji/snt/analysis/graph/package-summary.html>`__.
"""
...
CURATED_CLASSES = [
    "AnnotationGraph",
    "DirectedWeightedGraph", "DirectedWeightedSubgraph",
    "SNTGraph"
]
...
class AnnotationGraph:
    """
    A graph implementation for brain (neuropil) annotations
    """
    pass
...
class DirectedWeightedSubgraph:
    """
    Class for accessing sub-graphs of a reconstruction.
    """
    pass

class SNTGraph:
    """
    Class providing color mapping functionality for graph visualization.
    """
    pass
...
def _java_setup():
    """
    Lazy initialization function for Java-dependent graph analysis classes.
    ...
    """
    global _curated_classes, AnnotationGraph, DirectedWeightedGraph, DirectedWeightedSubgraph, SNTGraph

    try:
        package_name = "sc.fiji.snt.analysis.graph"
        ...
        if "AnnotationGraph" in _curated_classes:
            globals()["AnnotationGraph"] = _curated_classes["AnnotationGraph"]
        if "DirectedWeightedGraph" in _curated_classes:
            globals()["DirectedWeightedGraph"] = _curated_classes["DirectedWeightedGraph"]
        if "DirectedWeightedSubgraph" in _curated_classes:
            globals()["DirectedWeightedSubgraph"] = _curated_classes["DirectedWeightedSubgraph"]
        if "SNTGraph" in _curated_classes:
            globals()["SNTGraph"] = _curated_classes["GraphUtils"]
        
        logger.info(f"Successfully loaded {len(_curated_classes)} curated graph analysis classes")
        
    except Exception as e:
        logger.error(f"Failed to load curated graph analysis classes: {e}")
        raise ImportError(f"Could not load curated graph analysis classes: {e}") from e
...
def _discover_extended_classes():
    ...
    try:
        ...
        logger.info(f"Discovered {len(_extended_classes)} extended graph analysis classes")
...
def list_classes():
    ...
    print("Available SNT Graph Analysis Classes:")
    print("=" * 40)
...
def __getattr__(name: str) -> Any:
    """
    Provide dynamic access to graph analysis classes.
...
class AnnotationGraph:
    """
    Curated SNT class from analysis/graph package with complete method signatures.
